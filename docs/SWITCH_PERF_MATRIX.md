...
See also **Performance analogy (TPT → NXSand)** in [`TPT_IMPORT.md`](TPT_IMPORT.md). Battery Saver sets runtime `water_levelRate` to **0.010** (reduced wide/pocket fill) via `applyPerfPresetPhysics` (does not auto-save `physics.json`).
...
These add branch-light `cell()` reads only when `water_levelRate > 0` and on reaction checks. Active tiles remain **Conservative** by default, with full-grid fallback when row-run area exceeds the safety threshold and **sleep** after 30 idle frames when active tile count is zero (or empty grid with Active tiles Off).
