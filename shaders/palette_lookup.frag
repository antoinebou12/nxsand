...
vec3 materialColor(uint m, float n) {
    ...
    if (m == M_SMOKE) return mix(vec3(0.13, 0.13, 0.15), vec3(0.45, 0.47, 0.50), n);
...
void main() {
    ...
    float shade = openTop * 0.10 + openLeft * 0.04 - float(!openBottom) * 0.04;
