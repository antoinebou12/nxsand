...
bool canDisplace(uint moving, uint target) {
    ...
    if (target == M_WALL || isPlant(target) || target == M_ICE ||
        target == M_GLASS || target == M_WOOD || target == M_METAL)
        return false;
...
bool nearCardinalSmoke(ivec2 pos) {
    return neighborHas(cell(pos + ivec2( 1,  0)), cell(pos + ivec2(-1,  0)),
                       cell(pos + ivec2( 0,  1)), cell(pos + ivec2( 0, -1)), M_SMOKE);
}

float fallChanceAt(uint m, ivec2 pos) {
    if (m == M_FIRE && nearCardinalSmoke(pos))
        return clamp(fallChance(m) * 0.65, 0.25, 1.0);
    if (m == M_SAND && sand_wetSlideScale > 0.0 && sandNearWater(pos))
...
bool isSolidSurface(uint m) {
    return m == M_WALL || m == M_STONE || m == M_SAND || m == M_SALT || isPlant(m) || m == M_ICE ||
           m == M_GLASS || m == M_WOOD || m == M_METAL || m == M_BRICK;
}
...
float gasHorizFlow(uint gas, ivec2 pos, ivec2 dir) {
    ...
    if (gas == M_FIRE && nearCardinalSmoke(pos)) ch *= 0.55;
...
uint react(uint self, ivec2 c) {
    if (self == M_LEGACY_TNT || self > 22u) return M_EMPTY;
    ...
    if (self == M_EMPTY) {
        ...
        if (nearPlant) {
            ...
            bool woodSupport = plant_wallSupport >= 0.5 &&
                (neighborHas(nRight, nLeft, nUp, nDown, M_WOOD) || countDiagonal(c, M_WOOD) > 0);
            bool woodClimb = woodSupport && (below == M_WOOD || isPlant(below));
            float chance = 0.0;
            ...
            if (woodSupport)
                chance = max(chance, min(1.0, plant_growthRate * 4.5));
            if (woodClimb)
    ...
    if (self == M_SAND) {
        if (nearLava && nearWater && rand01(c, 52u) < 0.42) return M_STONE;
        if (nearLava && rand01(c, 16u) < 0.55) return M_GLASS;
        if (nearAcid && rand01(c, 17u) < 0.12) return M_EMPTY;
        if (sand_lithifyRate > 0.0) {
            int waterN = countCardinal(nRight, nLeft, nUp, nDown, M_WATER);
            if (waterN >= 1) {
                float lith = sand_lithifyRate;
                if (waterN >= 3) lith = min(1.0, lith * 6.0);
                else if (waterN >= 2) lith = min(1.0, lith * 3.0);
                else lith = min(1.0, lith * 1.5);
                if (rand01(c, 51u) < lith) return M_STONE;
            }
        }
        return M_SAND;
    }
