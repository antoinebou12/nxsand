...
void run_materials_tests(TestContext& ctx) {
    ...
    CHECK(ctx, pal[nx::MAT_TNT] == nx::pack_rgb(180, 50, 45));
    ...
    CHECK(ctx, std::strcmp(nx::material_short_name(nx::MAT_TNT), "TNT") == 0);
    CHECK(ctx, std::strcmp(nx::material_name(nx::MAT_COAL), "Coal") == 0);
    CHECK(ctx, std::strcmp(nx::material_name(nx::MAT_TNT), "TNT") == 0);
    ...
    CHECK(ctx, nx::PLAYABLE_MATERIALS.size() == 20);
    ...
    CHECK(ctx, nx::PLAYABLE_MATERIALS[17] == nx::MAT_TNT);
    CHECK(ctx, nx::PLAYABLE_MATERIALS[18] == nx::MAT_SALT);
    CHECK(ctx, nx::PLAYABLE_MATERIALS[19] == nx::MAT_BRICK);
    CHECK(ctx, nx::PICKER_MATERIALS.size() == 20);
    CHECK(ctx, nx::HUD_PALETTE_ROW2.size() == 6);
    CHECK(ctx, nx::HUD_PALETTE_ROW3.size() == 6);
    ...
    CHECK(ctx, nx::sanitizeBrushMaterial(21) == nx::MAT_TNT);
