...
- Desktop play: mouse brush and WASD movement; material ring hover/confirm must use `PlayRegion` in `materialWheelLayout`/`input.cpp` (not full drawable—fixes offset hover); menu/HUD hints use desktop copy (`ui_copy`), not Switch Joy-Con strings.
...
- Switch controls: `switch_face.hpp` for confirm/back/ring; optional `NXSAND_SWITCH_SWAP_FACE_XY` env swaps ring face; menus navigable with D-pad/stick and touch. Play: materials via **ring only** (X / H desktop; no bottom HUD palette on any platform); touch paints sim and picks ring chips (A confirm). Switch **fullscreen sim** uses zero play insets (`playHudInsets` → full drawable height; compact top HUD drawn over the sand). On stock Mesa: no **First-time setup** copy, no `shader_cache/` on SD; boot shows **Ready** and defers `sim.frag` compile to first New/Load/Demo.
